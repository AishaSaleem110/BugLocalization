...
def check_matchings(src_files, bug_reports):
    ...
    for report in bug_reports.values():
      #Checking all source files for each bug report
        matched_count = []
        ...
        for src in src_files.values():
          #set is ordered and cannot contain duplicate elements while list can
          #Convert stemmed summary of report into a set and source file's name into a set, after that take intersection of two sets 
          #to find the common tokens and find the length of common tokens.
            if src.file_name['stemmed']:
                common_tokens = len(set(summary_set['stemmed']) & 
                                    set([src.file_name['stemmed'][0]]))

            #if exact match between source code file name with bug report summary then length will be one, hence one point score is given 
            #to the source file.
            matched_count.append(common_tokens)

        #For this bug report if no exact match with any source file name is found then checking other parts of source files:
