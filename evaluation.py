...
def evaluate(src_files, bug_reports, coeffs, *rank_scores):
    ...
    for i, (bug_id, report) in enumerate(bug_reports.items()):
        ...
        # MAP
          ...
          result_file.write(str(bug_id) + ',' + ','.join(src_ranks) + '\n')
        except:
          pass
    result_file.close()
...
def main():
  ...
  #Mention here only those components that you want to combine in total scores
  #All five components are included for now
  #You don't need to change code from anywhere else, just don't include it in calculation here
